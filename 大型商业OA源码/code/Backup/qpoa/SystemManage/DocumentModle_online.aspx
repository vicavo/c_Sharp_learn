<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DocumentModle_online.aspx.cs" Inherits="qpoa.SystemManage.DocumentModle_online" %>
...
<HTML>
	<HEAD>
		...
		function chknull()
		{
			...
			var num=Math.random();
			document.all.FramerControl1.Open("http://"+serverpath+"/SystemManage/DocumentModle/<%=forfile%>?tmp="+num+"", true);
        ...
        function SaveToWeb()
        {
          showwait();	
           ...
           document.all.FramerControl1.HttpPost("http://"+serverpath+"/SystemManage/DocumentModle_online_save.aspx?file=<%=forfile%>&number=<%=number%>&forname=<%=forname%>");
        ...
        function OpenToWeb(){
           var serverpath=window.location.host+v_online_path; 
           document.all.FramerControl1.InSertFile("http://"+serverpath+"/images/logo.jpg",8);
        }	      
										...
										<tr>
											...
											<td width="52%">
                                                <input id="Button2" type="button" value="关　闭" OnClick="window.close()"/>
                                                   <input id="Button3" type="button" value="1111" OnClick="OpenToWeb()"/>
                                                </td>
										</tr>
									</table>			
